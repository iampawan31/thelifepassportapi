    ...
    <div class="c">
        <div v-if="showFamilyDetails">
            ...
            <div class="data">
                ...
                <div class="item clearfix" v-for="(family, index) in familyDetails" v-bind:key="index">
                    <div class="item__name"><strong>{{ family.legal_name }}</strong></div>
                    <div class="item__relationship">{{ family.family_relation.title }}</div>
            ...
            <form id="frmFamilyMemberSection" name="frmFamilyMemberSection" method="post" class="custom-form">
                ...
                <div class="field-group clearfix">
                    ...
                    <router-link to="/family-members" class="btn-primary btn-editinfo">Add Member</router-link>
                </div>
            </form>
        </div>
        <div class="question-item" v-if="!showFamilyDetails">
            <div class="question-header">
                ...
                <div class="yesno">
                    <a href="javascript:void(0)" @click.prevent="familymemberstatus(1)" class="btn-yes">Yes</a>
                    <a href="javascript:void(0)" @click.prevent="familymemberstatus(0)" class="btn-no">No</a>
                </div>
            </div>
            <a href="javascript:void(0)" @click.prevent="familymemberstatus(2)" class="btn-skip">Skip</a>
        </div>
    </div>
</template>
<script>
export default {
    ...
    methods: {
        ...
        familymemberstatus(status) {
            ...
            axios.post('familyinfo/updatefamilystatus', this.formData)
            .then((response) => {
                if (status == '1') {
