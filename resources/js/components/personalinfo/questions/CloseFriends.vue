    ...
    <div class="c">
        <div v-if="showFriendsDetails">
            ...
            <div class="data">
                ...
                <div class="item clearfix" v-for="(friend, index) in friendsDetails" v-bind:key="index">
                    <div class="item__name"><strong>{{ friend.legal_name }}</strong></div>
                    <div class="item__email"> {{ friend.email }} </div>
                    <div class="item__phone"></div>
                    <div class="item__action">
                        <router-link :to="{ path: '/close-friends/' + friend.id }" class="btn-edit">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                        </router-link>&nbsp;
                        <a href="javascript:void();" @click="removeFriend(friend.id)" class="btn-delete">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
                        </a>
                    </div>
                </div>
            </div>
            <form id="frmFriendsSection" name="frmFriendsSection" method="post" class="custom-form" @submit.prevent="handleSubmit">
                ...
                <div class="field-group clearfix">
                    ...
                    <router-link to="/close-friends" class="btn-primary btn-editinfo">Add Member</router-link>
                </div>
            </form>
        </div>
        <div class="question-item" v-if="!showFriendsDetails">
            <div class="question-header">
                ...
                <div class="yesno">
                    <a href="javascript:void(0)" @click.prevent="friendsstatus(1)" class="btn-yes">Yes</a>
                    <a href="javascript:void(0)" @click.prevent="friendsstatus(0)" class="btn-no">No</a>
                </div>
            </div>
            <a href="javascript:void(0)" @click.prevent="friendsstatus(2)" class="btn-skip">Skip</a>
        </div>
    </div>
</template>
<script>
export default {
    ...
    methods: {
        async handleSubmit(e) {
            ...
            axios.post("/friendsinfo/updatestatus", formData)
                .then(response => {
        ...
        friendsstatus(status) {
            ...
            axios.post('friendsinfo/updatefriendsstatus', this.formData)
            .then((response) => {
                if (status == '1') {
            ...
            axios.get('/friendsinfo/getfriendsinfo').then((response) => {
                if (response.status == 200) {
                    if (response.data.data[0]) {
                        this.friendsDetails = JSON.parse(JSON.stringify(response.data.data));
        ...
        removeFriend(id) {
            axios.delete('friendsinfo/'+id+'/removefriends')
                .then((response) => {
                    if (response.status == 200) {
                        this.getFriendsInfo(); 
                    }
